...
class EpisodicReplayBuffer:
    ...
    def insert(self, data: Dict, episode_index: int) -> None:
        ...
        # check if the episode data contains NaN in the reward key
        if np.isnan(data['reward']).any():
            print("Episode data contains NaN in the reward key")
    ...
    def sample(self, batch_size: int, sequence_length: int) -> Dict:
        ...
        return jax.tree.map(lambda x: np.swapaxes(x[sequence_inds], 0, 1), self.data)
