    ...
    def update(
        ...
        *,
        ...
        def world_model_loss_fn(
            ...
            # jax.debug.print("consistency_loss: {consistency_loss}", consistency_loss=consistency_loss)
            # jax.debug.print("reward_loss: {reward_loss}", reward_loss=reward_loss)
            # jax.debug.print("value_loss: {value_loss}", value_loss=value_loss)
            # jax.debug.print("continue_loss: {continue_loss}", continue_loss=continue_loss)
        ...
        if not self.model.crossq:
            ...
            new_target_value_model = self.model.target_value_model
    ...
    def td_target(
        ...
        Qs, _ = self.model.Q(
            ...
            (
                self.model.target_value_model.params
                if not self.model.crossq
                else self.model.value_model.params
            ),
            key=dropout_key,
        )
