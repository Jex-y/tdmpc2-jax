...
from typing import Callable, Optional, Sequence, Any, Callable, Optional, Sequence, Tuple, Union
...
class BatchRenorm(nn.Module):
    ...
    Copied from Stable Baselines JAX
    ...
    use_running_average: Optional[bool] = False
    axis: int = -1
    momentum: float = 0.99
    epsilon: float = 0.001
...
class NormedLinear(nn.Module):
  ...
  def __call__(self, x: jax.Array, training: bool = True) -> jax.Array:
    ...
    if self.use_batch_norm:
      x = BatchRenorm(dtype=self.dtype)(x)
